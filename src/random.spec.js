...
test(`shuffle should shuffle a list`, (t) => {
...
test(`floor should return a random number floored`, (t) => {
...
test(`floorMin should return a random number floored with a minimum`, (t) => {
  t.is(typeof floorMin, `function`)
  const [a, b, c] = map(floorMin(100), [1e5, 1e5, 1e5])
  t.false((a === b) && (b === c))
})

test(`pick should take unwrapped values from arrays`, (t) => {
...
test(`pick should take unwrapped values from objects`, (t) => {
...
test(`grab should take wrapped values from arrays`, (t) => {
  const input = characters
...
test(`word should return a random concatenation of letters`, (t) => {
...
test(`wordSource should return a random concatenation of letters given a source`, (t) => {
  const out = wordSource([`a`], 10)
  t.is(out, `aaaaaaaaaa`)
})

test(`grab should take wrapped values from objects`, (t) => {
  const input = charactersAsObjectList
  const picked = grab(input)
...
test(`allot should randomly take a given number of values from a list`, (t) => {
